    ...
    <header className="border-b bg-background sticky top-0 z-50" style={{ boxShadow: "0 2px 4px rgba(0,0,0,0.08)" }}>
      <div className="flex items-center h-[72px] px-4 lg:px-6 gap-2">
        ...
        <Link to="/" className="flex-shrink-0 mr-2">
          <span className="text-2xl font-black tracking-tight text-udemy-heading">
            udemy
          </span>
        </Link>
        ...
        <div className="hidden lg:block relative group">
          ...
          <div className="absolute left-0 top-full pt-2 hidden group-hover:block z-50">
            <div className="bg-background border shadow-lg w-64 py-2">
              {categories.map((cat) => (
                <Link
                  key={cat}
        ...
        <div className="flex items-center gap-1 ml-auto">
          <Link
            to="/courses"
            className="hidden lg:block text-sm text-muted-foreground hover:text-primary px-3 py-2"
          >
            Teach on Udemy
          </Link>
          ...
          <div className="flex items-center gap-2 ml-2">
            <Link to="/login">
              <Button variant="udemy-outline" size="sm" className="text-sm h-10 px-4">
                Log in
              </Button>
            </Link>
            <Link to="/register">
              <Button variant="udemy" size="sm" className="text-sm h-10 px-4">
                Sign up
              </Button>
            </Link>
            <button className="hidden lg:flex items-center justify-center w-10 h-10 border border-foreground">
